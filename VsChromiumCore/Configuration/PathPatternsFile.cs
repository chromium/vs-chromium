...
namespace VsChromiumCore.Configuration {
  public class PathPatternsFile : IPathPatternsFile {
    ...
    public PathPatternsFile(IConfigurationSectionProvider configurationSectionProvider, string configurationFileName) {
      this._configurationSectionProvider = configurationSectionProvider;
      this._configurationFileName = configurationFileName;
      this._matcherLines = new Lazy<List<PathMatcher>>(CreateMatcherLines, LazyThreadSafetyMode.PublicationOnly);
      this._matcher = new Lazy<IPathMatcher>(CreateMatcher, LazyThreadSafetyMode.PublicationOnly);
    }
    ...
    public IEnumerable<IPathMatcher> GetPathMatcherLines() {
      return this._matcherLines.Value;
    }

    private List<PathMatcher> CreateMatcherLines() {
      var patterns = this._configurationSectionProvider.GetSection(this._configurationFileName, FilterDirectories);
      return patterns.Select(x => PatternParser.ParsePattern(x)).ToList();
    }

    private IPathMatcher CreateMatcher() {
      return new AggregatePathMatcher(this._matcherLines.Value);
    }
