...
namespace VsChromium.Core.Collections {
  public class SortedArray<T> : IList<T>, ICollection<T> {
    ...
    public int BinaraySearch<TKey>(TKey item, Func<T, TKey, int> itemComparer) {
      return BinarySearch(_items, 0, _items.Length, item, itemComparer);
    }

    public static int BinarySearch<TKey>(T[] array, int index, int length,  TKey item, Func<T, TKey, int> itemComparer) {
