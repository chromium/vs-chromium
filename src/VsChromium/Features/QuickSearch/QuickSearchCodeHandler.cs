...
namespace VsChromium.Features.QuickSearch {
  ...
  public class QuickSearchCodeHandler : IViewHandler {
    ...
    public void Attach(IVsTextView textViewAdapter) {
      ...
      var target = new SimpleCommandTarget(
        new CommandID(GuidList.GuidVsChromiumCmdSet, (int)PkgCmdIdList.CmdidQuickSearchCode),
        Execute);
      var targetWrapper = new OleCommandTarget("QuickSearchCode", target);
